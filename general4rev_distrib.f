...
ccc
	...
	g = 9.81
        dip = pi*(90.0-dip)/180.
        open(49, file=outfile)

	do iz=0, 24
	  zdep = iz*0.5
	  s3 = rho*g*zdep*10.0**(-3)
...
c        s3 = rho*g*5.0*10.0**(-3)
c	  call general4(s3, fs, fd, cohesive, tvalue, phi, dsg, tb) 
c	  s1 = s3 + 2.*dsg 
c        write(49, '(a5,2f10.3)') "p", (s1+s3)/2., 0.0
c        write(49, '(a5,2f10.3)') "s1", s1, 0.0
c        write(49, '(a5,2f10.3)') "s3", s3, 0.0
c        write(49, '(a5,2f10.3)') "fs", 0.0, cohesive+fs*0.0
c        write(49, '(a5,2f10.3)') "fs", 100.0, cohesive+fs*100.0
c        write(49, '(a5,2f10.3)') "fs", 200.0, cohesive+fs*200.0
c        write(49, '(a5,2f10.3)') "fs", 300.0, cohesive+fs*300.0
c        write(49, '(a5,2f10.3)') "fd", 0.0, fd*0.0
c        write(49, '(a5,2f10.3)') "fd", 100.0, fd*100.0
c        write(49, '(a5,2f10.3)') "fd", 200.0, fd*200.0
c        write(49, '(a5,2f10.3)') "fd", 300.0, fd*300.0
c        do i=0,180
c          tau0 = dsg*sin(pi*real(i)/180.) 
c          sn0 = (s1+s3)/2. - dsg*cos(pi*real(i)/180.)
c          write(49, '(a5,2f10.3)') "mohr", sn0, tau0
c        enddo
      
c        tau0 = dsg*sin(2.d0*phi) 
c        sn0 = (s1+s3)/2. - dsg*cos(2.0*phi)
c        write(49, '(a5,2f10.3)') "phi", phi/pi*180.
c        write(49, '(a5,2f10.3)') "tpop", sn0, cohesive+sn0*fs
c        write(49, '(a5,2f10.3)') "t0op", sn0, tau0
c        write(49, '(a5,2f10.3)') "trop", sn0, sn0*fd
      
c        tau0 = dsg*sin(2.d0*dip) 
c        sn0 = (s1+s3)/2. - dsg*cos(2.0*dip)
c        write(49, '(a5,2f10.3)') "dip", (pi/2.-dip)/pi*180.
c        write(49, '(a5,2f10.3)') "tp", sn0, cohesive+sn0*fs
c        write(49, '(a5,2f10.3)') "t0", sn0, tau0
c        write(49, '(a5,2f10.3)') "tr", sn0, sn0*fd
